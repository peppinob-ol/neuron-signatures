"""
POC 3.4: Advanced gradient-guided swap + manual causal interventions + write-score inspection.

Upgrades over POC 3.3:
1. Manual causal mode (--force_neurons): bypass selection, intervene on specific neurons
   with ablate/add/set/flip modes. Useful for hypothesis testing on individual neurons.
2. Write-score inspection (--inspect_write_scores): estimate what tokens a neuron "pushes"
   via W_out[i] @ W_U. Quick structural check before/after causal tests.

Preserved from POC 3.3:
- Multi-position patching
- Stronger metrics (rank, margin, prob)
- Controls (structured, permuted, scrambled, shuffled-neurons)
- Auto layer selection
- Paper-grade JSON reporting

Run examples:

  # === POC 3.4 NEW: Manual causal interventions ===
  
  # A) Ablate a specific neuron (test if neuron 3428 is "say(Austin)")
  python -m neuron_signatures.poc3_4_grad_guided_swap_advanced \\
    --prompt "The capital of the state containing Dallas is" \\
    --source_prompt "The capital of the state containing San Francisco is" \\
    --target_token " Austin" --alt_token " Sacramento" \\
    --layer 18 --pos_mode last --pos -1 \\
    --force_neurons "3428" --force_mode ablate

  # B1) Add negative delta to neuron (should boost Austin if it's a "say(Austin)" suppressor)
  python -m neuron_signatures.poc3_4_grad_guided_swap_advanced \\
    --prompt "The capital of the state containing Dallas is" \\
    --source_prompt "The capital of the state containing San Francisco is" \\
    --target_token " Austin" --alt_token " Sacramento" \\
    --layer 18 --pos_mode last --pos -1 \\
    --force_neurons "3428" --force_mode add --force_value -1.0

  # B2) Add positive delta to neuron (should suppress Austin if above holds)
  python -m neuron_signatures.poc3_4_grad_guided_swap_advanced \\
    --prompt "The capital of the state containing Dallas is" \\
    --source_prompt "The capital of the state containing San Francisco is" \\
    --target_token " Austin" --alt_token " Sacramento" \\
  ...
  # === POC 3.4 NEW: Write-score inspection ===
  
  # Inspect what tokens neuron 3428 "writes to" via W_out @ W_U
  python -m neuron_signatures.poc3_4_grad_guided_swap_advanced \\
    --prompt "The capital of the state containing Dallas is" \\
    --source_prompt "The capital of the state containing San Francisco is" \\
    --target_token " Austin" --alt_token " Sacramento" \\
    --layer 18 --pos_mode last --pos -1 \\
    --force_neurons "3428" --force_mode ablate \\
    --inspect_write_scores --inspect_topk 25 \\
    --inspect_tokens " Austin| Sacramento| capital| Dallas| Texas"

  # === POC 3.3 preserved: Gradient-guided swap ===
  
  # Paper-style pos=-1 with controls
  python -m neuron_signatures.poc3_4_grad_guided_swap_advanced \\
    --prompt "The capital of the state containing Dallas is" \\
    --source_prompt "The capital of the state containing San Francisco is" \\
...
# -----------------------------------------------------------------------------
# POC 3.4: Manual causal helpers
# -----------------------------------------------------------------------------

def compute_acts_multi_pos_no_grad(
    model,
    tokens: torch.Tensor,
    layer: int,
    positions: List[int],
    ...
    """
    Capture MLP activations at multiple positions WITHOUT gradient tracking.
    Returns list of [d_mlp] tensors (float32, CPU) for each position.
    """
    hook_name = f"blocks.{layer}.mlp.hook_post"
    captured: Dict[str, torch.Tensor] = {}

    def capture(act: torch.Tensor, hook):
        captured["act"] = act.detach()
        return act

    model.reset_hooks(including_permanent=True)
    with torch.inference_mode():
        _ = model.run_with_hooks(tokens, fwd_hooks=[(hook_name, capture)], return_type="logits")
    model.reset_hooks(including_permanent=True)

    if "act" not in captured:
        raise RuntimeError(f"Hook did not capture activation for {hook_name}")

    act_full = captured["act"]  # [1, seq, d_mlp]
    return [act_full[0, p].float().cpu() for p in positions]


def inspect_write_scores(
    model,
    layer: int,
    neuron_idxs: List[int],
    topk: int,
    explicit_token_strs: Optional[List[str]] = None,
) -> Dict[str, Any]:
    """
    Compute write scores for neurons: what tokens do they "push" via W_out @ W_U?
    
    This is a structural proxy (ignores LayerNorm, attention, other MLP contributions)
    but is useful as a quick hypothesis check.
    
    Returns dict with per-neuron top_pos, top_neg tokens and optional explicit scores.
    """
    # Grab weights
    try:
        W_out = model.blocks[layer].mlp.W_out.detach()  # [d_mlp, d_model]
    except Exception as e:
        raise RuntimeError(f"Cannot access W_out for layer {layer}: {e}")

    W_U = getattr(model, "W_U", None)
    if W_U is None:
        raise RuntimeError("Model has no W_U (unembed) available; cannot inspect write scores.")

    device = W_U.device
    WU = W_U.detach().float()  # [d_model, vocab]
    V = W_out[torch.tensor(neuron_idxs, dtype=torch.long, device=W_out.device)].detach().float()  # [n, d_model]
    V = V.to(device=device)

    # scores[n, vocab] = V @ W_U
    scores = V @ WU  # [n, vocab]

    out: Dict[str, Any] = {"layer": layer, "neurons": []}
    ...
    for j, nidx in enumerate(neuron_idxs):
        ...
        # explicit tokens
        if explicit_token_strs:
            explicit = []
            for ts in explicit_token_strs:
                tid = resolve_single_token_id(model, ts)
                explicit.append({
                    "token_str": ts,
                    "id": int(tid),
        ...
        out["neurons"].append(neuron_entry)
...
def run_experiment(
    ...
    force_neurons: Optional[str],  # POC 3.4: forced neuron mode
    force_mode: str,
    force_value: float,
    topk_n: int,
) -> Dict[str, Any]:
    ...
    # Parse forced neurons if provided
    forced: Optional[List[int]] = None
    if force_neurons is not None:
        forced = [int(x.strip()) for x in force_neurons.split(",") if x.strip()]
    
    is_forced = (forced is not None)
    
    # Build plans - either forced or gradient-guided
    if forced is not None:
        # Manual causal mode: no selection, no grads, just force these neurons
        acts_dest = compute_acts_multi_pos_no_grad(model, dest_tokens, layer, dest_positions)
        
        plans: List[PosPlan] = []
        for dest_pos, a_dest in zip(dest_positions, acts_dest):
            idx_t = torch.tensor(forced, dtype=torch.long)
            a_dest_sel = a_dest[idx_t].clone()
            
            if force_mode == "ablate":
                deltas = -a_dest_sel
            elif force_mode == "flip":
                deltas = -2.0 * a_dest_sel
            elif force_mode == "add":
                deltas = torch.full_like(a_dest_sel, float(force_value))
            elif force_mode == "set":
                deltas = torch.full_like(a_dest_sel, float(force_value)) - a_dest_sel
            else:
                raise ValueError(f"Unknown force_mode: {force_mode}")
            
            k = len(forced)
            zeros = torch.zeros(k, dtype=torch.float32)
            plans.append(PosPlan(
                dest_pos=dest_pos,
                src_pos=-1,
                neuron_idxs=forced,
                deltas=deltas,
                grads=zeros,
                pred_terms=zeros,
                pred_sum=0.0,  # pred not defined in forced mode
                a_dest_sel=a_dest_sel,
            ))
        
        baseline_diff = float("nan")  # not meaningful in forced mode
    else:
        # Gradient-guided mode (original POC 3.3 behavior)
    ...
    result = {
        "success": True,
        "mode": "forced" if is_forced else "grad_guided",
        "layer": layer,
        ...
        "k_per_pos": k_per_pos,
        "alpha": alpha,
        "selection_mode": selection_mode if not is_forced else f"forced:{force_mode}",
        "pred_effect": total_pred if not is_forced else None,  # meaningless in forced mode
        ...
        "pred_vs_actual": pred_vs_actual if not is_forced else None,  # meaningless in forced mode
        "abs_error": abs_error,
        "plans": [
            {
                "dest_pos": p.dest_pos,
    ...
    # Add forced mode info if applicable
    if is_forced:
        result["forced_info"] = {
            "neurons": forced,
            "mode": force_mode,
            "value": force_value if force_mode in ["add", "set"] else None,
            "a_dest_vals": [
                [float(v) for v in p.a_dest_sel.tolist()]
                for p in plans
            ],
        }
    ...
    # DISABLED in forced mode (no meaningful grads/selection to transfer)
    # -------------------------------------------------------------------------
    if not is_forced and ctrl_cache is not None and ctrl_positions is not None:
    ...
    # DISABLED in forced mode
    # -------------------------------------------------------------------------
    if not is_forced and enable_permute_control and len(plans) > 0 and permute_trials > 0:
        perm_d_diffs = []
    ...
    # DISABLED in forced mode
    # -------------------------------------------------------------------------
    if not is_forced and len(plans) > 1 and scramble_trials > 0:
        scramble_d_diffs = []
    ...
    # DISABLED in forced mode
    # -------------------------------------------------------------------------
    if not is_forced and enable_shuffle_neuron_control and len(plans) > 0 and shuffle_neuron_trials > 0:
        shuffle_d_diffs = []
        base_seed = torch.initial_seed()
...
def auto_pick_best_layer(
    ...
    for layer in tqdm(layers, desc="Layer sweep", ascii=True):
        ...
        result = run_experiment(
            ...
            enable_shuffle_neuron_control=False, shuffle_neuron_trials=0,
            force_neurons=None, force_mode="ablate", force_value=0.0, topk_n=5
        )
...
def main():
    ap = argparse.ArgumentParser(description="POC 3.4: Gradient-guided swap + manual causal + write-score inspection")
    ...
    # Forced neuron interventions (POC 3.4)
    ap.add_argument("--force_neurons", default=None,
                    help="Comma-separated neuron idxs to force (bypass selection), e.g. '3428' or '3428,123'")
    ap.add_argument("--force_mode", default="ablate",
                    choices=["ablate", "add", "set", "flip"],
                    help="ablate: add -a_dest; flip: add -2*a_dest; add: add constant delta; set: set to value.")
    ap.add_argument("--force_value", type=float, default=0.0,
                    help="Used by force_mode=add (delta) or set (absolute value).")
    
    # Write-score inspection (POC 3.4)
    ap.add_argument("--inspect_write_scores", action="store_true",
                    help="Inspect neuron write scores to tokens via W_out and W_U.")
    ap.add_argument("--inspect_layer", type=int, default=None,
                    help="Layer for write-score inspection (defaults to --layer if set).")
    ap.add_argument("--inspect_neurons", default=None,
                    help="Comma-separated neurons to inspect (defaults to --force_neurons if set).")
    ap.add_argument("--inspect_topk", type=int, default=25,
                    help="Top-k tokens to show for positive/negative write scores.")
    ap.add_argument("--inspect_tokens", default=None,
                    help="Pipe-separated token strings to score explicitly, e.g. ' Austin| Sacramento| capital'.")
    ...
    # =========================================================================
    # POC 3.4: Write-score inspection (optional)
    # =========================================================================
    write_scores_payload = None
    if args.inspect_write_scores:
        layer_ins = args.inspect_layer if args.inspect_layer is not None else (args.layer if args.layer is not None else None)
        if layer_ins is None:
            raise ValueError("--inspect_write_scores requires --inspect_layer or --layer")
        
        neurons_str = args.inspect_neurons or args.force_neurons
        if not neurons_str:
            raise ValueError("--inspect_write_scores requires --inspect_neurons or --force_neurons")
        
        neuron_idxs = [int(x.strip()) for x in neurons_str.split(",") if x.strip()]
        explicit = [s.strip() for s in args.inspect_tokens.split("|")] if args.inspect_tokens else None
        
        safe_print(f"=== Write-score inspection: layer={layer_ins}, neurons={neuron_idxs} ===")
        write_scores_payload = inspect_write_scores(model, layer_ins, neuron_idxs, args.inspect_topk, explicit)
        
        # Human-readable output
        for n in write_scores_payload["neurons"]:
            safe_print(f"\n[write-scores] L{layer_ins} neuron {n['neuron_idx']}")
            safe_print("  top_pos: " + ", ".join([f"{t['token']}({t['score']:+.3f})" for t in n['top_pos'][:10]]))
            safe_print("  top_neg: " + ", ".join([f"{t['token']}({t['score']:+.3f})" for t in n['top_neg'][:10]]))
            if "explicit" in n:
                safe_print("  explicit: " + ", ".join([f"{e['token_str']}({e['score']:+.3f})" for e in n['explicit']]))
        safe_print("")
        ...
        for k in k_values:
            for alpha in alpha_values:
                result = run_experiment(
                    ...
                    args.force_neurons, args.force_mode, args.force_value,
                    args.topk_tokens
                )
                ...
                if is_sweep:
                    ...
                    pva_str = f"p/a={result['pred_vs_actual']:.2f}" if result['pred_vs_actual'] is not None else "forced"
                    safe_print(f"L{layer:2d} K={k:3d} a={alpha:.1f}: d_diff={result['d_logit_diff']:+.4f} {pva_str}{ctrl_info}")
                else:
                    # Detailed single-run output
                    safe_print(f"\n--- Layer {layer}, K={k}, alpha={alpha} ---")
                    safe_print(f"Mode: {result.get('mode', 'grad_guided')}")
                    safe_print(f"Positions: {result['n_positions']}/{result['requested_positions']}, Total neurons: {result['total_neurons']}")
                    
                    # Pred effect only meaningful in grad_guided mode
                    if result['pred_effect'] is not None:
                    ...
                    # Pred/actual only meaningful in grad_guided mode
                    if result['pred_vs_actual'] is not None:
                        safe_print(f"  pred/actual  = {result['pred_vs_actual']:.3f}")
                        safe_print(f"  abs_error    = {result['abs_error']:+.6f}")
                    
                    # Show forced mode info if applicable
                    if "forced_info" in result:
                        fi = result["forced_info"]
                        safe_print(f"\nForced mode: {fi['mode']} on neurons {fi['neurons']}")
                        if fi['value'] is not None:
                            safe_print(f"  force_value: {fi['value']}")
                        safe_print(f"  a_dest values: {fi['a_dest_vals']}")
    ...
    if successful:
        ...
        safe_print(f"\nBest: L{best['layer']} K={best['k_per_pos']} a={best['alpha']} (mode={best.get('mode', 'grad_guided')})")
        safe_print(f"  d_logit_diff = {best['d_logit_diff']:+.6f}")
        if best['pred_vs_actual'] is not None:
    ...
    # POC 3.4: Add forced mode info to payload
    if args.force_neurons:
        payload["forced_mode"] = {
            "neurons": [int(x.strip()) for x in args.force_neurons.split(",") if x.strip()],
            "mode": args.force_mode,
            "value": args.force_value if args.force_mode in ["add", "set"] else None,
        }
    
    # POC 3.4: Add write-score inspection to payload
    if write_scores_payload is not None:
        payload["inspect_write_scores"] = write_scores_payload
